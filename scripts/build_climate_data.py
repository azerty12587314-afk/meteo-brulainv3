...
def get(a,b):
 p={'latitude':LOC['latitude'],'longitude':LOC['longitude'],'start_date':a,'end_date':b,'daily':','.join(V),'timezone':LOC['timezone'],'models':'era5'}
 last=None
 for n in range(6):
  try:
   ...
   if 'precipitation_sum' not in daily:
    raise RuntimeError(f'precipitation_sum absente pour {a}-{b}')
   return j
...
def normals(rs):
 ...
 for m in range(1,13):
  a=agg[m]
  out.append({'month':m,'label':calendar.month_abbr[m],**{k:mean([float(x[k]) for x in a if x.get(k) is not None and math.isfinite(float(x[k]))]) for k in ['temperatureMean','temperatureMaxMean','temperatureMinMean','precipitation','sunshineHours','evapotranspiration','frostDays','summerDays','hotDays']}})
 return out
def annual(y,rs):
 tm=[num(x,'temperature_2m_mean') for x in rs]
 return {'year':y,'temperatureMean':mean(tm),'precipitation':summ([num(x,'precipitation_sum') for x in rs]),'sunshineHours':summ([num(x,'sunshine_duration')/3600 for x in rs]),'evapotranspiration':summ([num(x,'et0_fao_evapotranspiration') for x in rs]),'frostDays':sum(num(x,'temperature_2m_min')<0 for x in rs),'summerDays':sum(num(x,'temperature_2m_max')>=25 for x in rs),'hotDays':sum(num(x,'temperature_2m_max')>=30 for x in rs),'tropicalNights':sum(num(x,'temperature_2m_min')>=20 for x in rs),'dryDays':sum(math.isfinite(num(x,'precipitation_sum')) and num(x,'precipitation_sum')<0.1 for x in rs),'rainDays':sum(math.isfinite(num(x,'precipitation_sum')) and num(x,'precipitation_sum')>=1 for x in rs),'heatingDegreeDays':summ0([max(0,18-x) for x in tm if math.isfinite(x)]),'monthly':monthly(rs)}
...
def main():
 ...
 for y in range(max(2021,cy-5),cy+1):
  try:
   r=rows(get(f'{y}-01-01',(date.today()-timedelta(days=5)).isoformat() if y==cy else f'{y}-12-31')); rr+=r; recent.append(annual(y,r));time.sleep(2)
  except Exception as e:errors.append(str(e))
 ...
 normal_rain=[m.get('precipitation') for m in out['normals']['monthly']]
 if not any(isinstance(v,(int,float)) and math.isfinite(v) and v>0 for v in normal_rain):
  out['errors'].append('Aucune précipitation mensuelle valide reçue; ancien climate.json à conserver.')
  print(json.dumps(out,ensure_ascii=False,indent=2))
  return 1
 Path('data/climate.json').write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8')
 print(json.dumps(out,ensure_ascii=False,indent=2))
 return 0
